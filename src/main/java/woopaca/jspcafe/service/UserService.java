...
public class UserService {
    ...
    public UserProfile getUserProfile(Long userId) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new NotFoundException("[ERROR] 사용자를 찾을 수 없습니다."));
    ...
    public User updateUserProfile(Long userId, UpdateProfileRequest updateProfileRequest, Authentication authentication) {
        ...
        if (!user.matchPassword(updateProfileRequest.password())) {
            throw new UnauthorizedException("[ERROR] 비밀번호가 일치하지 않습니다.");
        }
    ...
    private void validateDuplicateUsername(String username) {
        userRepository.findByUsername(username)
                .ifPresent(user -> {
                    throw new BadRequestException("[ERROR] 이미 사용 중인 이메일입니다.");
                });
    }
    ...
    private void validateDuplicateNickname(String nickname) {
        userRepository.findByNickname(nickname)
                .ifPresent(user -> {
                    throw new BadRequestException("[ERROR] 이미 사용 중인 닉네임입니다.");
                });
    }
}
