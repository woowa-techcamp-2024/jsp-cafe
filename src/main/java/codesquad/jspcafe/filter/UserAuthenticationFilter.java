...
/**
 * UserAuthenticationFilter는 로그인을 하지 않은 사용자가 접근하는 URI에 접근하는 경우를 처리하는 필터입니다. <br> 로그인을 하지 않은 사용자가
 * 접근하는 URI에 접근하는 경우 로그인 페이지로 리다이렉트합니다.
 */
@WebFilter(urlPatterns = {"/users", "/questions", "/questions/*"})
public class UserAuthenticationFilter extends HttpFilter {

    /**
     * 로그인을 하지 않은 사용자가 접근하는 URI에 접근하는 경우 로그인 페이지로 리다이렉트합니다.
     *
